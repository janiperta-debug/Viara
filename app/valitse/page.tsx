...
export default async function ValitsePage() {
  ...
  if (rooli === "admin") redirect("/admin/kayttajat");
  ...
  return (
    <main className="flex min-h-screen flex-1 items-center justify-center px-6 py-16">
      <div className="w-full max-w-sm">
        <h1 className="mb-2 text-center text-2xl font-bold text-foreground">Valitse näkymä</h1>
        <p className="mb-8 text-center text-sm text-muted">{kayttaja.rooliLabel}</p>
          ...
          {nakymat.map(({ href, label }) => {
            ...
            return (
              <form key={href} action={asetaNakyma.bind(null, nakyma)}>
                <button type="submit" className="btn-primary flex h-14 w-full items-center justify-center rounded-full text-base font-semibold text-primary-foreground">
                  {label}
                </button>
              </form>
            );
