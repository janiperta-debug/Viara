...
> {
  ...
  const organisaatioId =
    typeof kayttaja.organisaatio_id === "string"
      ? kayttaja.organisaatio_id
      : null;
...
function validoiSalasana(salasana: string): string | null {
  const value = salasana;
  if (value.length < 8 || value.length > 200) return null;
  return value;
}

/**
 * Viara-admin luo uuden organisaation ja sen ensimmäisen työnjohtajan.
 * Auth-käyttäjä luodaan palvelinpuolen service role -asiakkaalla.
 */
export async function luoOrganisaatioJaEnsimmainenTyonjohto(input: {
  ...
  if (!salasana) {
    return { ok: false, virhe: "Salasanan tulee olla vähintään 8 merkkiä." };
  }
  ...
  try {
    ...
    if (authError || !authData.user) {
      ...
      return {
        ok: false,
        virhe: authError?.message ?? "Auth-käyttäjän luonti epäonnistui.",
      };
    }
...
/**
 * Työnjohto lisää oman organisaationsa uuden käyttäjän.
 * Työnjohto voi luoda vain kuljettajan tai toisen työnjohtajan.
 */
...
/**
 * Nostaa oman organisaation kuljettajan työnjohdoksi.
 * Adminia ei voi luoda tällä actionilla eikä organisaatiota voi vaihtaa.
 */
export async function nostaKuljettajaTyonjohdoksi(
  ...
  if (hakuError || !kohde) {
    return { ok: false, virhe: "Käyttäjää ei löytynyt." };
  }
