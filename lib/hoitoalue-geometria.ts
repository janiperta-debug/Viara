...
export type HoitoalueGeoJSON = GeoJSONPolygon | GeoJSONMultiPolygon;

const MAAPALLON_SADE_METREINA = 6_371_000;

function pistePolygonissa(lat: number, lng: number, polygon: number[][][]): boolean {
  ...
  function pisteRenkaassa(ring: number[][]): boolean {
    ...
    for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
      ...
      const lngJ = ring[j][0];
      const latJ = ring[j][1];
      const leikkaa = latI > lat !== latJ > lat && lng < ((lngJ - lngI) * (lat - latI)) / (latJ - latI) + lngI;
  ...
  for (const reika of polygon.slice(1)) if (pisteRenkaassa(reika)) return false;
  return true;
}

function etaisyysPisteestaJanaanMetreina(lat: number, lng: number, alku: number[], loppu: number[]): number {
...
function etaisyysPolygoninRajastaMetreina(lat: number, lng: number, polygon: number[][][]): number {
  let pienin = Number.POSITIVE_INFINITY;
  for (const ring of polygon) {
    if (ring.length < 2) continue;
    for (let i = 0; i < ring.length - 1; i += 1) {
      pienin = Math.min(pienin, etaisyysPisteestaJanaanMetreina(lat, lng, ring[i], ring[i + 1]));
    }
  }
  return pienin;
}

export function pisteGeoJSONissa(lat: number, lng: number, geojson: HoitoalueGeoJSON, lasnaoloalueMetrit: number): boolean {
  ...
  const sisallaPolygonissa = geojson.coordinates.find((polygon) => pistePolygonissa(lat, lng, polygon));
  if (lasnaoloalueMetrit >= 0) {
    if (sisallaPolygonissa) return true;
    return geojson.coordinates.some((polygon) => etaisyysPolygoninRajastaMetreina(lat, lng, polygon) <= toleranssi);
  }
