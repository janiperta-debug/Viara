...
function escapePdfText(text: string) { return text.replace(/\\/g, "\\\\").replace(/\(/g, "\\(").replace(/\)/g, "\\)"); }
...
export function muodostaRaporttiPdf(data: PdfRaportti) {
  ...
  objects.push(objectBytes(["<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>"])); objects.push(objectBytes(["<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>"]));
  for (let i = 0; i < pageCount; i++) { const pageId = 5 + i * 2; const contentId = pageId + 1; const stream = winAnsi(makePage(lines, i)); objects.push(objectBytes([`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_WIDTH} ${PAGE_HEIGHT}] /Resources << /Font << /FR 3 0 R /FB 4 0 R >> >> /Contents ${contentId} 0 R >>`])); objects.push(objectBytes([`<< /Length ${stream.length} >>\nstream\n`, stream, "\nendstream"])); }
